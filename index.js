  ...
  apply(compiler) {
    compiler.hooks.compilation.tap('GoogleTagManagerPlugin', (compilation) => {
      HtmlWebpackPlugin.getHooks(compilation).beforeEmit.tapAsync(
        'GoogleTagManagerPlugin', (htmlPluginData, callback) => {
          const headRegExp = /(<\/head>)/i
          const bodyRegExp = /(<body\s*>)/i

          if (this.options.dataLayer)
            htmlPluginData.html = htmlPluginData.html.replace(headRegExp, match => this.snippets.dataLayer + match)
          htmlPluginData.html = htmlPluginData.html.replace(headRegExp, match => this.snippets.script + match)
          htmlPluginData.html = htmlPluginData.html.replace(bodyRegExp, match => match + this.snippets.noScript)
